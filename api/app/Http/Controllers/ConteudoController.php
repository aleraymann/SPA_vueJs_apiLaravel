...
class ConteudoController extends Controller
{
    ...
    public function pagina($id, Request $request)
    {
        ...
        if($donoDaPagina){
        $conteudos = $donoDaPagina->conteudo()->with('user')->orderBy('data','DESC')->paginate(5);;   
        $user = $request->user();
        ...
        foreach($conteudos as $key => $conteudo){
            ...
            return ['status'=>true, 'erro'=>"Usuario nao existe!"]; 
        }
    ...
    if($conteudo){
        ...
        return ['status'=>false, 'erro'=>"Conteúdo Inexistente!"];
    }
